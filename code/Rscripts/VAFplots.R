...
for (mut in c("12868G>A", "2788C>A", "3209A>G")){
	png(paste0("Lareau_VAF",substr(mut, 1, nchar(mut)-3),".png"))
...
## Our results
plot_AF <- function(variant, pbmc1_af, pbmc1_umap, bm1_af, bm1_umap, pbmc2_af, 
                    pbmc2_umap, bm2_af, bm2_umap){
...
for (mut in c("12868G>A", "2788C>A", "3209A>G")){
	png(paste0("Daphne_VAF",substr(mut, 1, nchar(mut)-3),".png"))
	p <- plot_AF(mut,
						PBMC1_AF,
						PBMC1_UMAP,
						BM1_AF,
