...
svg("mtDepth_plot.svg")
p <- ggplot(SO@meta.data, aes(x="", y=mtDNA_depth)) +
	geom_violin(fill = "grey") +
	geom_jitter(aes(color = mtDNA_depth > as.numeric(args[4])), 
	...
	theme(legend.position = "none", text = element_text(size = 20),
    		...
    		axis.text = element_text(size = 16)) +
	geom_hline(yintercept = as.numeric(args[4]), color = "black", linetype = 2) +
	ggtitle("mtDNA depth") +
	scale_y_continuous(trans='log10')
print(p)
dev.off()
...
# svg("Circular_mtDepth_before.svg")
...
# 
# svg("Circular_mtDepth_after.svg")
# d = d[d$V2 %in% colnames(SO), ]
